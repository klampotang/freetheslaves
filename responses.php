<?php
    ...
    // start the session
    session_start();
    //session_unset();//free session variables, uncomment to do once and recomment

    //If session var counter does not exist create it
    if (!isset($_SESSION['counter'])) {
    	$_SESSION['counter'] = 0;
    	
	}
	//else set it to current counter
	$counter = $_SESSION['counter'];


    // If language var does not exist create it
	...
	//if they have already gone through all the questions, reset to beginning
    if($counter >= 11) {
		$counter = 0;
	}

    // increment counter to track question
    $counter++;
    
    //initialize default response message
	$respmessage = "";
	
	//Check if the last message id exists
	...
	//get the last message using the message id
	$sms = $client
    ->messages($message_id)
    ->fetch();
    $last_message = $sms->body;

    //check counter for what response to give
    //first counter: ask language
    if($counter == 1) {
    ...
    //save the language and counter variables across sessions
